...
export const SUMMARY_SYSTEM = `你是一個團隊 retro（回顧會議）的 AI 助理。你會收到一場 retro 的所有回答（已去識別化，只有文字）。
...
export function buildContext(
  ...
  for (const q of questions) {
    ...
    lines.push(`### ${q.label}`);
    if (group.length === 0) lines.push("（沒有回答）");
...
export async function geminiSummary(context: string): Promise<string> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) throw new Error("尚未設定 GEMINI_API_KEY，無法產生 AI 報告。");
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      contents: [
        { role: "user", parts: [{ text: "請根據以上回答產生總結與建議。" }] },
      ],
  ...
  const reply: string =
    data?.candidates?.[0]?.content?.parts
      ...
      .join("")
      .trim() ?? "";
  if (!reply) throw new Error("AI 沒有回覆內容，請再試一次。");
  return reply;
}
